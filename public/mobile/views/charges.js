(() => {
  ...
  registry.charges = ({ runtimeState, esc }) => {
    ...
    const cards = rows.map(row => `
      <button type="button" class="v5-record-card" data-record-type="charge" data-record-id="${esc(row.id)}" aria-label="查看扣费项目 ${esc(row.name)} 详情">
    ...
    return `
      <section class="v5-mobile-view v5-core-view" data-mobile-view="charges" aria-labelledby="v5MobileViewTitle">
        <div class="v5-mobile-view-heading"><div><span class="v5-mobile-eyebrow">AMAZON CHARGES</span><h1 id="v5MobileViewTitle">扣费</h1><p>按费用名称查看净成本，而不是宽表逐列找</p></div><button class="v5-mobile-period-trigger" type="button" data-mobile-action="period" aria-label="选择查看期间"><span>${esc(m.rangeLabel)}</span><i aria-hidden="true">›</i></button></div>
