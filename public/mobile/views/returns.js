  ...
  registry.returns = ({ runtimeState, esc }) => {
    ...
    return `
      <section class="v5-mobile-view v5-core-view" data-mobile-view="returns" aria-labelledby="v5MobileViewTitle">
        ...
        <section class="v5-secondary-hero"><span>退货原因记录</span><strong>${fmt.number(m.total)}</strong><div><small>${fmt.number(m.rows.length)} 个原因</small></div></section>
        <section class="v5-core-section"><div class="v5-core-section-head"><div><span>REASONS</span><h2>主要退货原因</h2></div><small>${rows.length ? `Top ${rows.length}` : '月级明细'}</small></div><div class="v5-secondary-list">${cards || '<div class="v5-core-empty"><strong>没有退货原因明细</strong><span>完整月份导入后会显示 return reason 数据。</span></div>'}</div></section>
      </section>`;
  };
})();
